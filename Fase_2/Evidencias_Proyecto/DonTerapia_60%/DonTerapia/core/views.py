...
from .models import Actividad, UsuarioPersonalizado, ActividadRealizada, TareaAsignada,RespuestaTarea,Feedback
...
def realizar_feedback_acc(request, actividad_realizada_id):
    # Obtener la actividad realizada específica de "ANÁLISIS DE CONDUCTA (ACC)"
    actividad_realizada = get_object_or_404(ActividadRealizada, id=actividad_realizada_id)
    ...
    if request.method == 'POST':
        ...
        return redirect('progreso_pacientes')  # Redirige a la página de progreso de pacientes
...
def realizar_feedback_tarea(request, respuesta_tarea_id):
    # Obtener la respuesta específica de la tarea
    respuesta_tarea = get_object_or_404(RespuestaTarea, id=respuesta_tarea_id)
    ...
    return render(request, 'app/realizar_feedback.html', {
        'actividad_titulo': respuesta_tarea.tarea.actividad.titulo,
        'pregunta': respuesta_tarea.tarea.descripcion,
